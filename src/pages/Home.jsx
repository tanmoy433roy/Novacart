...
const Home = () => {
  ...
  useEffect(() => {
    if (imagesLoaded && carouselRef.current) {
      carouselRef.current.style.transform = `translateX(-${currentSlide * 100}%)`;
    }
  }, [currentSlide, imagesLoaded]);
    ...
    <div className="home">
      ...
      <div className="content-wrapper">
        <h2 className="home-title">
          {username ? (
            <>
              Welcome <span className="username">{username}</span> to{' '}
              <span className="brand-name">NovaCart</span>
            </>
          ) : (
            <>
      ...
      <footer className="footer">
        <div className="footer-content">
          <div className="footer-section">
            <h3>About Novacart</h3>
            <p>
              Novacart Technologies LLC is your premier destination for all
              electronics needs. We offer the latest gadgets, computers, and
              tech accessories with the best prices and quality service.
            </p>
          </div>
          <div className="footer-section">
            <h3>Quick Links</h3>
